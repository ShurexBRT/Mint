...
export const opportunities = sqliteTable('opportunities', {
  ...
  evidenceMode: text('evidence_mode', { enum: ['SYNTHETIC', 'LIVE', 'MIXED'] }).notNull().default('SYNTHETIC'),
  demandScore: real('demand_score').notNull().default(0),
  urgencyScore: real('urgency_score').notNull().default(0),
  willingnessToPayScore: real('willingness_to_pay_score').notNull().default(0),
  competitionScore: real('competition_score').notNull().default(0),
  buildComplexityScore: real('build_complexity_score').notNull().default(0),
  distributionScore: real('distribution_score').notNull().default(0),
  recurringScore: real('recurring_score').notNull().default(0),
  ...
  palermo: text('palermo', { mode: 'json' }).$type<PalermoResult>(),
...
export const evidence = sqliteTable('evidence', {
  ...
  sourceType: text('source_type', { enum: ['forum', 'review', 'issue', 'interview', 'research'] }).notNull(),
  excerpt: text('excerpt').notNull(),
  signalType: text('signal_type', {
    enum: ['pain', 'urgency', 'willingness_to_pay', 'alternative', 'distribution', 'recurring', 'feasibility', 'commodity', 'legal_risk', 'security_risk', 'privacy_risk', 'platform_risk'],
  }).notNull(),
  ...
  synthetic: integer('synthetic', { mode: 'boolean' }).notNull().default(true),
  complexityEstimate: integer('complexity_estimate'),
...
export const decisions = sqliteTable('decisions', {
  ...
  detail: text('detail', { mode: 'json' }).$type<DirectorResult>().notNull(),
  createdAt: text('created_at').notNull(),
});
...
export const experiments = sqliteTable('experiments', {
  ...
  status: text('status', { enum: ['ACTIVE', 'KILLED'] }).notNull().default('ACTIVE'),
  synthetic: integer('synthetic', { mode: 'boolean' }).notNull().default(true),
  createdAt: text('created_at').notNull(),
...
export const metrics = sqliteTable('metrics', {
  ...
  check('metric_demo_only', sql`${t.synthetic} = 1 AND ${t.verified} = 0`),
  check('metric_nonnegative', sql`${t.visits} >= ${t.signups} AND ${t.signups} >= ${t.purchases} AND ${t.purchases} >= 0 AND ${t.revenue} >= 0`),
]);
...
export const costEntries = sqliteTable('cost_entries', {
  ...
  category: text('category', { enum: ['AI_API', 'EXTERNAL'] }).notNull(),
...
export const distributionExperiments = sqliteTable('distribution_experiments', {
  ...
  status: text('status').notNull().default('REQUIRES_OWNER_APPROVAL'),
  createdAt: text('created_at').notNull(),
});
...
export const researchSignals = sqliteTable('research_signals', {
  ...
  source: text('source', { enum: ['github', 'hackernews'] }).notNull(),
  sourceUrl: text('source_url').notNull(),
  sourceType: text('source_type', { enum: ['issue', 'forum'] }).notNull(),
  title: text('title').notNull(),
  excerpt: text('excerpt').notNull(),
  ...
  index('research_signals_collected_idx').on(t.collectedAt),
  index('research_signals_type_idx').on(t.signalType),
]);

export const researchClusters = sqliteTable('research_clusters', {
  id: text('id').primaryKey(),
  title: text('title').notNull(),
  problemStatement: text('problem_statement').notNull(),
  targetUser: text('target_user').notNull(),
  signalIds: text('signal_ids', { mode: 'json' }).$type<string[]>().notNull(),
  keywords: text('keywords', { mode: 'json' }).$type<string[]>().notNull(),
  sourceCount: integer('source_count').notNull(),
  domainCount: integer('domain_count').notNull(),
  status: text('status', { enum: ['PROPOSED', 'PROMOTED', 'REJECTED'] }).notNull().default('PROPOSED'),
  createdAt: text('created_at').notNull(),
  updatedAt: text('updated_at').notNull(),
});
