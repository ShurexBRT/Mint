...
export function challenge(
  ...
  const risks = all.filter(e =>
    ['legal_risk', 'security_risk', 'privacy_risk', 'platform_risk'].includes(e.signalType),
  );
  ...
  if (commodities.length) reasons.push('Commodity pressure exists; differentiation must be proven before BUILD.');
  if (!buying) reasons.push('No independent evidence of willingness to pay.');
  if (duplicates.length) {
    reasons.push(
      `${duplicates.length} duplicate evidence item(s) excluded. Source identity is asserted, not externally verified.`,
    );
  }
  if (!reasons.length) reasons.push('No rule-based veto found. This is not proof of demand or a legal review.');

  return PalermoSchema.parse({
    verdict: risks.length ? 'VETO' : commodities.length || !buying || duplicates.length ? 'CAUTION' : 'PASS',
    critical: risks.length > 0,
    reasons,
    evidenceIds: [...risks, ...commodities].map(e => e.id),
  });
}
