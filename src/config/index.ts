...
export const RulesSchema = z.object({
  ...
  .refine(r => r.buildScore >= r.validateScore, 'buildScore must be at least validateScore')
...
export const RadarConfigSchema = z.object({
  cooldownMinutes: z.number().int().min(5).max(1440).default(10),
  maxQueries: z.number().int().min(1).max(4).default(4),
  perSourceLimit: z.number().int().min(1).max(5).default(5),
}).strict();
...
export function readConfig(env: NodeJS.ProcessEnv = process.env) {
  ...
  return {
    port: base.PORT,
    databasePath: base.DATABASE_PATH,
    spendLimit: SPEND_LIMIT,
    rules: RulesSchema.parse(env.MINT_RULES_JSON ? JSON.parse(env.MINT_RULES_JSON) : {}),
    radar: RadarConfigSchema.parse(env.MINT_RADAR_JSON ? JSON.parse(env.MINT_RADAR_JSON) : {}),
  };
}
